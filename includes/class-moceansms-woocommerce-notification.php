...
class Moceansms_WooCoommerce_Notification {
    ...
    public static function send_customer_notification($order_id, $status) {
        ...
        if($phoneNumberUtil->isValidNumber($phoneNumber) && $phoneNumberUtil->getNumberType($phoneNumber) == 1) {
            ...
            $log->add('MoceanSMS', 'Customer\'s billing phone number ('.$order_details->billing_phone.') in country ('.$order_details->billing_country.') converted to '.$customer_phone_no);

            self::send_sms($customer_phone_no, $message);            
        } else {
            $log->add('MoceanSMS', 'Customer\'s billing phone number ('.$order_details->billing_phone.') not a valid mobile number in country ('.$order_details->billing_country.'), not sending SMS.');
        }
    }
